...
export class ShoppingListModal extends BaseModal implements OnInit {
    ...
    public isDataValid(): boolean {
        return this.products.length > 0 &&
            this.hasEveryProductValidPriceAndQuantity() &&
            this.hasEveryProductValidName() &&
            this.dateService.isDateValid(this.shoppingDate) &&
            this.place.length > 0;
    }
    ...
    public onAddShoppingListClick() {
        ...
        this.shoppingListToAdd
            .prices
            .forEach((price, i) => price.productId = this.getProductIdForName(this.productNamesForPrices[i]));

        this.pricesService
            .addShoppingList(this.shoppingListToAdd)
            .first()
            .subscribe(
    ...
    private getProductIdForName(productName: string): number {
        return this.products
            ...
            .pop() || DOES_NOT_CONTAIN;
    }

    private hasEveryProductValidPriceAndQuantity(): boolean {
        return this.shoppingListToAdd
            .prices
            .every(price => price.value > 0 && price.quantity > 0);
    }

    private hasEveryProductValidName(): boolean {
        return this.productNamesForPrices
            .every(productName => this.getProductIdForName(productName) !== DOES_NOT_CONTAIN);
    }
}
