...
export class CycleAddModal extends BaseModal implements OnInit, AfterViewInit {
    ...
    public isDataValid(): boolean {
        return this.setsToAdd.length > 0 &&
            this.hasEverySetName() &&
            this.dateService.isDateValid(this.startDate);
    }

    public onAddClick(): void {
        this.cycleToAdd = new NewCycle(
            this.dateService.parseStringToDate(this.startDate),
            this.setsToAdd
        );

        this.trainingsService
            .addCycle(this.cycleToAdd)
            .first()
            .subscribe(
                ok => {
    ...
    public canModalBeOpened(): boolean {
        ...
        Materialize.toast(
            this.dictionaryService.getDictionaryValue('page.trainings.cycle.finishAll.tooltip'),
            3000
        );
        return false;
    }
    ...
    public trackByIndex(index: number, set: NewSet): number {
        return index;
    }
