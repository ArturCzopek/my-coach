...
export class TrainingsBackEndService extends TrainingsService {
  ...
  public getActiveCycle(): Observable<Cycle> {
    return this.http.get(`${environment.server.url}${this.trainingUrl}${this.cycleUrl}/active`, this.userService.prepareAuthOptions()).map(res => res.json());
  }

  public getCyclePreviews(): Observable<CyclePreview[]> {
    return this.http.get(`${environment.server.url}${this.trainingUrl}${this.cycleUrl}/previews`, this.userService.prepareAuthOptions()).map(res => res.json());
  }

  public getCycle(cycleId: number): Observable<Cycle> {
    return this.http.get(`${environment.server.url}${this.trainingUrl}${this.cycleUrl}/${cycleId}`, this.userService.prepareAuthOptions()).map(res => res.json());
  }

  public getExercisesWithSessionForTraining(trainingId: number): Observable<Exercise[]> {
    return this.http.get(`${environment.server.url}${this.trainingUrl}${this.exerciseUrl}/${trainingId}`, this.userService.prepareAuthOptions()).map(res => res.json());
  }

  public addCycle(cycleToAdd: NewCycle): Observable<any> {
    return this.http.post(`${environment.server.url}${this.trainingUrl}${this.cycleUrl}/add`, cycleToAdd, this.userService.prepareAuthOptions());
  }

  public addExercises(exerciseToAdd: NewExercise[]): Observable<any> {
    return this.http.post(`${environment.server.url}${this.trainingUrl}${this.exerciseUrl}/add`, exerciseToAdd, this.userService.prepareAuthOptions());
  }

  public addTraining(trainingToAdd: NewTraining): Observable<any> {
    return this.http.post(`${environment.server.url}${this.trainingUrl}/add`, trainingToAdd, this.userService.prepareAuthOptions());
  }
  ...
  public editCycle(cycleToEdit: Cycle): Observable<any> {
    return this.http.put(`${environment.server.url}${this.trainingUrl}${this.cycleUrl}/update`, cycleToEdit, this.userService.prepareAuthOptions());
  }

  public editExercise(exerciseToEdit: Exercise): Observable<any> {
    return this.http.put(`${environment.server.url}${this.trainingUrl}${this.exerciseUrl}/update`, exerciseToEdit, this.userService.prepareAuthOptions());
  }
  ...
  public hasUserOnlyFinishedCycles(): Observable<boolean> {
    return this.http.get(`${environment.server.url}${this.trainingUrl}${this.cycleUrl}/finished`, this.userService.prepareAuthOptions()).map(res => res.json());
  }
}
